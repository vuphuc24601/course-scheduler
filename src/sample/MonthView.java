...
//            }
    ...
    static int tinder(int day,int crrMonth,int crrYear) {
        /**
         ...
         */
        for (int i = 1; i <= findMonthLimit(crrMonth,crrYear); i++)
            if (getDayOfTheWeek(i, crrMonth, crrYear) == day)
                return i;
        return 0;
    }

    static sample.Date nextDate(sample.Date i) {
        sample.Date res = new sample.Date(0, 0, 0, i.Stime[0], i.Stime[1]);
        if (i.Sdate[0] == findMonthLimit(i.Sdate[1],i.Sdate[2])) {
            res.Sdate[0] = 1;
            if (i.Sdate[1] == 12) {
                res.Sdate[1] = 1;
...
//>>>>>>> bc008343e554bbc17d86c44032467fbb813d6eec
